...
# 이중선로 173kg 초음속? 감속비 없이 쌩 모터만 존재
# 이중선로 최대 펄스 2009~12, 최소 펄스 986~8
...
RAW_MIN         = 21
RAW_MAX         = 4095
RAW_CENTER      = 2200  # 기존 2600

# Manual 모드 최대 출력(%) — 필요시 100으로 올리면 풀스케일
MANUAL_MAX_PCT = 50
...
BRAKE_MS        = 120   # 역방향 브레이크 유지 시간(ms)
BRAKE_MIN_DUTY  = 30    # 너무 약하면 효과 없음 → 하한
BRAKE_MAX_DUTY  = 70    # 너무 세면 충격 ↑ → 상한
last_dir = 0
...
def duty_target_from_diff(diff_us: int, sc_state: int) -> int:
    ...
    if sc_state == 1:
        ...
        if diff_us > 0:
            A = diff_us
            if A <= span_pos_low:
                return (A * MAX_DUTY_LOW) // max(1, span_pos_low)  # 0..80
            else:
            ...
            A = -diff_us
            if A <= span_neg_low:
                return (A * MAX_DUTY_LOW) // max(1, span_neg_low)  # 0..80
            else:
...
while True:
    ...
    # ── LED 우선순위
    # 1) 비상(이미 위에서 처리) 2) 와이퍼 중앙 아님 3) 저전압 4) 항상 켜기
    # 저전압일 경우에는 단순히 led 점멸만 정지 없음
    if alarm_active:
        led_code = 2   # 2번 점멸
    elif bat_pct < 20:
        led_code = 3   # 3번 점멸
    ...
    if m == 1:
        ...
        if utime.ticks_diff(now_ms, reader_rf.last_ms) > RF_TIMEOUT_MS:
            ...
            rf_duty_smooth = 0
            rf_pw_smooth = MID_POINT
            # print("EMERGENCY STOP: RF signal lost!")
            utime.sleep_ms(100)
            continue
        ...
        if sc_state == 1:
            rf_pw_eff_in = rf_pw
            if rf_pw_eff_in < LOW_MIN:
                rf_pw_eff_in = LOW_MIN
            elif rf_pw_eff_in > LOW_MAX:
                rf_pw_eff_in = LOW_MAX
        else:  # sc_state == 2
            rf_pw_eff_in = rf_pw
